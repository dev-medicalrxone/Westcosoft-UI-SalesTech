...
uses
  Windows, Messages, SysUtils, Classes, Dialogs; //CommObjs,Dialogs;
...
type
  ...
  public
    constructor Create(Handle: THandle; Events: TCommEventTypes); Virtual;
  ...
  public
    constructor Create(Owner: TCustomComm); virtual;
  ...
  protected
    ...
    property BaudRate: TBaudRate read FBaudRate write SetBaudRate default br2400;
    property Parity: TParity read FParity write SetParity default paEven;
    property Stopbits: TStopbits read FStopbits write SetStopbits default sb10;
    property Databits: TDatabits read FDatabits write SetDatabits default da7;
    property EventChars: TCommEventChars read FEventChars;
    property Options: TCommOptions read FOptions write SetOptions;
    property FlowControl: TFlowControl read FFlowControl write SetFlowControl default fcDTR;
  ...
  Private
    Save                 : Boolean;
    LineData, OldCommand : String;
    Procedure WhenRecieve(Sender: TObject; Count: Integer);
    function  LRC(Input: String): Integer;
    function  GetSpecificLenString(str : string;pSpecificLen : integer) : string;
    Function  ExecuteCommand(Command : String) : Boolean;
  Public
    Constructor Create(AOwner: TComponent); Override;
    Procedure   ShowMessage(pMessage: string);
    Procedure   SelectAccount(pMessage,GScreen : string);
    Procedure   GeneratePinBlock(pMessage1,pMessage2,pPAN,pWorkingKey:string;MasterKeyIndex :char);
    Procedure   GetAmount(pMessage:string);
    Procedure   DataEntry(pMessage:string);
    Procedure   ExecuteUnknowCommand(Command:string);
    Procedure   ReadCard(pMessage,GScreen : String; ReadTrack1,ReadTrack2 : Boolean);
    Procedure   DataEncryption(pMKeyIndex:char;pWorkingKey,pInputData:string);
...
Constructor TS7.Create(AOwner: TComponent);
begin
  inherited Create(Aowner);
  OnRxChar   := WhenRecieve;
  OldCommand := '';
end;

function TS7.LRC(Input: String): Integer;
Var I, TmpLRC: Integer;
begin
  TmpLRC := 0;
  For I := 1 to Length(Input) do TmpLRC := TmpLRC xor Ord(Input[I]);
  Result := TmpLRC;
end;

Function TS7.ExecuteCommand(Command : String) : Boolean;
  var LRCValue : string;
begin
  Command    := Command + chr(3);
  LRCValue   := IntToStr(LRC(Command));
  Command    := Command+chr(StrToInt(LRCValue));
  Command    := chr(2)+Command;
  purgeIN;
  purgeOut;
  Write(Command[1],length(Command));
  Result := true;
end;

Function TS7.GetSpecificLenString(str : string;pSpecificLen : integer) : string;
   Var Len,t : Integer;
begin
  len     := length(str);
  Result  := str;
  if len < pSpecificLen then begin
     for t := 1 to pSpecificLen - len do Result := Result + ' ';
  end else if len > pSpecificLen then
     Result := Copy(Str, 1, pSpecificLen);
end;

Procedure TS7.GetAmount(pMessage:string);
  var tmpMessage : String[16];
begin
  tmpMessage := GetSpecificLenString(pMessage,16);
  ExecuteCommand('98.'+tmpmessage);
end;

Procedure tS7.GeneratePinBlock(pMessage1,pMessage2,pPAN,pWorkingKey:string;MasterKeyIndex :char);
  var tmpMessage1   : String[16];
      tmpMessage2   : String[16];
      tmpWorkingKey : String[16];
      tmpPAN        : String[18];
begin
  tmpMessage1   := GetSpecificLenString(pMessage1,16);
  tmpMessage2   := GetSpecificLenString(pMessage2,16);
  tmpWorkingKey := GetSpecificLenString(pWorkingKey,16);
  tmpPAN        := GetSpecificLenString(pWorkingKey,18);
  ExecuteCommand('32'+'.'+MasterKeyIndex+tmpPAN+chr(28)+tmpWorkingKey+
                 tmpMessage1+'04'+'0'+chr(28)+tmpmessage2);
end;

Procedure TS7.SelectAccount(pMessage,GScreen : string);
  var tmpMessage : String[16];
      tmpscreen  : string[4];
begin
      tmpMessage := GetSpecificLenString(pMessage,16);
      tmpscreen  := GetSpecificLenString(Gscreen,4);
      ExecuteCommand('30.'+tmpmessage+chr(28)+tmpscreen);
end;

Procedure TS7.DataEntry(pMessage:string);
  var tmpMessage : String[16];
begin
   tmpMessage := GetSpecificLenString(pMessage,16);
   ExecuteCommand('36.'+tmpmessage+'0016'); //ultimos 2 numero = maximo caracteres
end;

procedure TS7.ShowMessage(pMessage : String);
  var tmpMessage : String[16];
begin
  tmpMessage := GetSpecificLenString(pMessage,16);
  ExecuteCommand('3C.'+tmpmessage);
end;

Procedure TS7.DataEncryption(pMKeyIndex:char;pWorkingKey,pInputData:string);
  var tmpWorkingKey, tmpInputData : string[16];
begin
   tmpWorkingKey := GetSpecificLenString(pWorkingKey,16);
   tmpInputData  := GetSpecificLenString(pInputData,16);
   ExecuteCommand('80.'+pMKeyIndex+tmpWorkingKey+tmpInputData);
end;


Procedure TS7.ExecuteUnknowCommand(Command:string);
begin
  ExecuteCommand(Command);
end;

Function TS7.ReadMessage : String;
begin
  result := OldCommand;
end;

Procedure TS7.ReadCard(pMessage,GScreen : String;ReadTrack1,ReadTrack2 : Boolean);
  var RTrack1,RTrack2 : char;
      tmpMessage : string[16];
begin
  if ReadTrack1 then
     RTrack1 := '1'
  else
  ...
  tmpMessage := GetSpecificLenString(pMessage,16);
  ExecuteCommand('3A.'+Rtrack1+RTrack2+'123'+tmpmessage+chr(28)+GScreen);
end;

Procedure TS7.WhenRecieve(Sender: TObject; Count: Integer);
type
...
begin
  ...
  GetMem(Buffer, ReadBufSize);
  try
    Fillchar(Buffer^, ReadBufSize, 0);
    ...
    if Bytes = -1 then begin
      ...
      for P := 0 to Bytes - 1 do begin
               Caracter  := CharBuf(Buffer^)[P];
               if caracter = #3 then begin
                 Save := false;
                 if linedata <> oldcommand then oldcommand := linedata;
                 linedata := '';
               end else if caracter = #2 then begin
                 save := true;
               end else  if save  then
                 LineData := LineData + Caracter;
      end;
    end;
  finally
    FreeMem(Buffer);
...
procedure Register;
begin
  RegisterComponents('Dennis Ortiz', [TComm]);
  RegisterComponents('Dennis Ortiz', [TS7]);
end;
